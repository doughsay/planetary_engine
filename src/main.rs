...
fn main() {
    ...
    app.insert_resource(BenchConfig)
        ...
        .add_systems(Startup, (setup_scene, setup_fps_counter, disable_time_cap))
...
fn disable_time_cap(mut time: ResMut<Time<Virtual>>) {
    // Effectively disable max_delta by setting it to a very large value (1 hour)
    // This ensures simulation stays synced to wall clock even at low FPS
    time.set_max_delta(Duration::from_secs(3600));
}
...
fn run_benchmark_flyby(
    ...
    if bench_state.start_time.is_none() {
        bench_state.start_time = Some(current_time);
    }

    let t = (current_time - bench_state.start_time.unwrap()) as f32;
    bench_state.timer = t;

    // Exit after 10s
    if t >= 10.0 {
        info!("[BENCH] Benchmark complete.");
        info!(
            "[BENCH] SUMMARY: Min FPS: {:.1} | Max FPS: {:.1} | Max Frame Time: {:.2}ms",
            bench_state.min_fps, bench_state.max_fps, bench_state.max_frame_time
        );
        std::process::exit(0);
    }

    let Ok((mut cam_transform, mut cam_state, mut cam_cell)) = camera_q.single_mut() else {
        return;
    };
...
fn log_benchmark_stats(
    ...
    // Track extremes (ignore first half second for warm-up)
    if bench_state.timer > 0.5 {
        if fps > 0.0 {
            bench_state.min_fps = bench_state.min_fps.min(fps);
            bench_state.max_fps = bench_state.max_fps.max(fps);
        }
        bench_state.max_frame_time = bench_state.max_frame_time.max(frame_time);
    }

    bench_state.log_timer += time.delta_secs();
    if bench_state.log_timer >= 0.5 {
        bench_state.log_timer -= 0.5;

        info!(
            "[BENCH] Time: {:.1}s | FPS: {:.1} (min: {:.1}) | Frame: {:.2}ms (max: {:.2}ms)",
            bench_state.timer, fps, bench_state.min_fps, frame_time, bench_state.max_frame_time
        );
    }
}
...
fn setup_scene(
...

    {
        ...
        grid_cmds.spawn_spatial((
            ...
            Transform::from_xyz(5000.0, 5000.0, 5000.0).looking_at(Vec3::ZERO, Vec3::Y),
        ));
    }
        ...
        planet_id = grid_cmds
            .spawn_grid_default((
                ...
                Orbit {
                    ...
                    initial_mean_anomaly: 3.0 * std::f64::consts::PI / 2.0,
                    parent: None,
                },
            ))
            .id();

        satellite_id = grid_cmds
            .spawn_grid_default((
                Satellite,
                Transform::default(),
                Visibility::default(),
...
fn camera_tracking_hotkeys(
    ...
    if let Some(target_pos) = target_pos {
        let Ok((mut cam_transform, cam_global, mut cam_state)) = camera_q.single_mut() else {
            return;
        };
